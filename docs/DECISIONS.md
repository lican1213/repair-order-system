...
### D-064: 二手家电只做展示橱窗，不做交易系统

**决策：** 二手家电功能作为 v1.3 实现，模块定位为“后台 admin 发布二手家电信息，客户公开页面浏览，客户通过电话咨询确认”，不做在线交易闭环。

**理由：**
- 二手家电与维修店业务相关，但交易仍适合电话沟通和线下确认
- 商品价格、成色和库存变化快，页面信息只能作为参考
- 在线下单、支付、购物车、客户留言、复杂库存和多规格 SKU 会显著增加维护成本
- 当前系统定位是轻量 H5 工单系统，不应扩展成商城系统

**边界：**
- 公开页只展示 `status=在售` 的商品；可选展示已售商品，但默认不展示
- 永远不展示下架商品
- 必须展示“页面信息仅供参考，具体以电话沟通确认为准”的免责声明
- 后台管理需要 admin JWT；客户不需要账号
- 该功能不得混入 v1.2，后续不得扩展成商城系统

### D-065: 二手家电使用 status 下架，不做硬删除

**决策：** 二手家电后台不提供硬删除。商品不可展示时使用 `status=下架`，售出后使用 `status=已售`。

**理由：**
- 避免老板误删已录入商品信息和图片路径
- 回滚时可隐藏入口或批量下架，不需要删除数据
- 与“小店少量商品展示”的场景一致，不引入复杂审计或恢复机制

### D-066: 二手家电 price 使用 TEXT

**决策：** `used_appliances.price` 使用 TEXT，不使用 REAL。

**理由：**
- 二手价格经常是“面议”“800元起”“含安装另算”等非纯数字描述
- 页面信息只作参考，最终以电话沟通确认为准
- 避免诱导系统往自动报价或交易结算方向扩展

### D-067: 二手家电图片复用 uploads 静态挂载，不做复杂图片清理

**决策：** 二手家电图片保存到 `backend/uploads/used/`，通过现有 `/uploads/*` 静态挂载访问。v1.3 不做图片硬删除或孤儿图片清理系统。

**理由：**
- 复用现有上传文件访问架构，部署和备份路径不变
- 当前商品数量少，少量孤儿图片可接受
- 不引入后台图片管理系统，避免偏离展示橱窗边界

### D-068: 上传按钮使用 useRef 编程式触发，不依赖 label 隐式关联

**决策：** 后台二手家电图片上传改为 `<div onClick>` + `useRef` 编程式触发 `fileInputRef.current?.click()`，不再使用 `<label>` 包裹 `<input type="file" className="hidden">` 的隐式关联模式。

**理由：**
- `<label>` 隐式关联 `hidden` input 在某些浏览器（特别是移动端）中不可靠，点击无反应
- `useRef` + `onClick` 编程式触发是跨浏览器最可靠的文件选择器激活方式
- 同时添加了 `role="button"` 和 `onKeyDown` 保证键盘可访问性

### D-069: 二手家电支持硬删除 + 定时清理脚本

**决策：** 新增 `DELETE /api/admin/used-appliances/{id}` 端点，仅允许删除 `status=下架` 的商品，同时清理关联图片文件。新增 `scripts/cleanup_used_appliances.py` 脚本，可清理已下架超过 N 天（默认 30 天）的商品及其图片，支持 `--dry-run`。

**理由：**
- 云服务器只有 40GB，长期积累的下架商品图片会占用大量空间
- 后台手动删除 + cron 定时清理双保险
- 仅限下架商品可删，防止误删在售商品
- 删除时同时清理数据库记录和磁盘图片文件

### D-070: ImagePreviewModal 支持多图 gallery 翻页

**决策：** `ImagePreviewModal` 新增 `images?: string[]` prop，支持左右箭头翻页、图片计数器和键盘左右箭头导航。与原有 `src` prop 向后兼容。
